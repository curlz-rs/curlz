use crate::cli::{Cli, Commands, RequestCli};

use curlz::data::{HttpHeaders, HttpMethod, HttpRequest};
use curlz::ops::{
    LoadBookmark, MutOperation, Operation, OperationContext, RunCurlCommand, SaveBookmark,
};
use curlz::workspace::Environment;

use anyhow::Context;
use clap::Parser;
use curlz::interactive;
use curlz::variables::Placeholder;
...
fn main() -> Result<()> {
    ...
    match Cli::parse() {
        Cli {
            ...
            let placeholders = parse_define_as_placeholders(&define);
            let env = create_environment(env_file, &placeholders)?;
            let mut ctx = OperationContext::new(env)?;

            let method = extract_method(&mut raw)
                .unwrap_or_else(|| HttpMethod::from_str(http_method.as_str()))?;
            ...
            let request = if let Some((_slug, bookmark)) = bookmark.as_ref().map(|slug| {
                let result = LoadBookmark::new(slug.as_str(), method.clone()).execute(&ctx);
                ...
                if let Some(bookmark) = bookmark {
                    ...
                    extract_url(&mut raw)
                        .context("No Bookmark nor URL was provided")
                        .map(|url| HttpRequest {
                            url,
                            method,
                            headers,
            ...
            RunCurlCommand::new(&request).execute(&mut ctx)?;
...
fn create_environment(env_file: PathBuf, placeholders: &[Placeholder]) -> Result<Environment> {
...
fn parse_define_as_placeholders(defines: &[impl AsRef<str>]) -> Vec<Placeholder> {
    parse_define(defines)
